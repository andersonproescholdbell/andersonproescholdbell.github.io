  ...
  methods: {
    ...
    start: function() {
      ...
      points.forEach((cords) => {
        this.drawCircle(cords.x, cords.y, 3, 'black');
      });

      //adding points
      this.active = true;
      var lastx = points[0].x, lasty = points[0].y, lastDir = 3, dots = 0, allDots = [];
      var loop = setInterval(() => {
        if (!this.active) {
          clearInterval(loop);
        }
        //***************try something where x and y can go in different directions****************
        allDots = [];
